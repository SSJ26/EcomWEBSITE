...
admin = ['upload_img', 'category']
buyer = ['contact', 'order_history', 'usercart']
seller = ['myproducts', 'contact', 'history_sellers']
...
def upload_img(request):
    ...
    return render(request, 'upload_img.html', {'form': form, 'roles': seller})
...
def myproducts(request):
    ...
    imagelist = []
    for category in categorylist:
        image = Product.objects.filter(
            created_by=request.user, category=category)
        imagelist.append(image)
...
def place_order(request, id):
    ...
    item = Product.objects.get(id=id)
    ordered_item = Buy.objects.create(
        product_id=item, user_id=request.user, seller_id=item.created_by)
